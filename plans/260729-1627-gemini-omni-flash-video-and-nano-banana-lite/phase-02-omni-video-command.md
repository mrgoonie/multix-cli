...
- On success, write a descriptive `.mp4` to `getOutputDir()`, copy it to `--output` when provided, and print both the saved file and new interaction ID prominently for the next edit.
...
1. Add failing command-level tests for registration/help, input preflight, single-video enforcement, previous-ID forwarding, output naming/copy semantics, and a no-video interaction.
...
- [ ] Omni and Veo use separate commands/endpoints.
- [ ] A generated interaction ID is printed and can be supplied in the next command.
- [ ] All unsupported inputs fail before upload/submission with actionable errors.
- [ ] Both output transports produce a local MP4 and `--output` copy behavior matches existing Gemini commands.
