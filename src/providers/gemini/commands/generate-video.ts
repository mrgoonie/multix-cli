...
import { ValidationError, ProviderError } from "../../../core/errors.js";
import { resolveKey } from "../../../core/env-loader.js";
import { getOutputDir } from "../../../core/output-dir.js";
import { httpJson } from "../../../core/http-client.js";
import { getDefaultModel, ASPECT_RATIOS, type AspectRatio } from "../models.js";
...
export function registerGenerateVideoCommand(parent: Command): void {
  parent
    ...
    .option("--reference-images <paths...>", "Reference images (first=opening frame, second=closing)")
    ...
    .action(async (opts: {
      ...
      if (!model.startsWith("veo-")) {
        throw new ValidationError(
          `Video generation requires a Veo model (got '${model}'). ` +
          "Use: veo-3.1-generate-preview, veo-3.0-generate-001, etc.",
        );
      }
      ...
      if (opts.referenceImages?.length) {
        for (const imgPath of opts.referenceImages.slice(0, 2)) {
          if (!fs.existsSync(imgPath)) throw new ValidationError(`Reference image not found: ${imgPath}`);
      ...
      while (!done && Date.now() < deadline) {
        ...
        if (!done) logger.debug(`Still generating... (${Math.round((deadline - Date.now()) / 1000)}s left)`);
      }

      if (!done) throw new ProviderError("Video generation timed out after 10 minutes", "gemini");
      if (finalOp.error) throw new ProviderError(`Veo error: ${JSON.stringify(finalOp.error)}`, "gemini");
      ...
      if (!videoUri) {
        throw new ProviderError(`Unexpected Veo response structure: ${JSON.stringify(finalOp.response)}`, "gemini");
      }
